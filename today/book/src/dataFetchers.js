 ...
 */
export async function fetchAndTransformDataForType(sourceType, env) {
    ...
    for (const dataSource of sources) {
        try {
            const rawData = await dataSource.fetch(env); // Pass sourceType to fetch
            const unifiedData = dataSource.transform(rawData, sourceType); // Pass sourceType to transform
...
export async function fetchAllData(env) {
    ...
    for (const sourceType in dataSources) {
        if (Object.hasOwnProperty.call(dataSources, sourceType)) {
            fetchPromises.push(
                fetchAndTransformDataForType(sourceType, env).then(data => {
 ...
 */
export async function fetchDataByCategory(env, category) {
    if (!dataSources[category]) {
        console.warn(`Attempted to fetch data for unknown category: ${category}`);
        return [];
    }
    return await fetchAndTransformDataForType(category, env);
}
